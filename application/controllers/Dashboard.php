...
class Dashboard extends CI_Controller { 
    ...
    public function barangay_chart()
    {
        $tot_pre_registered = 0;
        $tot_validated = 0;
        if( strtolower( $_POST['barangay_type'] ) == "all" ){
            $barangay_type = $this->config->item('barangay');
        }else if( strtolower( $_POST['barangay_type'] ) == "rural" ){
            $barangay_type = $this->config->item('rural');
        }else if( strtolower( $_POST['barangay_type'] ) == "urban" ){
            $barangay_type = $this->config->item('urban');
        }

        foreach ($barangay_type as $barangay) {
            $pre_registered = $this->record_model->barangay_record_for_pre_registered($barangay);
            if(!empty($pre_registered)){
                $tot_pre_registered += (int)$pre_registered[0]['tot'];
                $data['pre_registered'][]       = (int)$pre_registered[0]['tot'];
                $data['categories'][]           = $barangay;
            }else{
                $data['pre_registered'][]       = 0;
                $data['categories'][]           = $barangay;
            }

            $validated = $this->validated_model->barangay_record_for_validated($barangay);
            if(!empty($validated)){
                $tot_validated += (int)$validated[0]['tot'];
                $data['validated'][]       = (int)$validated[0]['tot'];
            }else{
                $data['validated'][]       = 0; 
            }
        }
